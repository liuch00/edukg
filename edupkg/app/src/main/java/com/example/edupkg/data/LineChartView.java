...
public class LineChartView extends View {
    ...
    private String[] stageStr = new String[] {"t6", "t5", "t4", "t3", "t2", "t1"};
    /** 横线左边距大小 */
    private float lineLeftPadding;
    /** X轴单元宽度 */
    private float unitWidth;
    /** 各个阶段数据数组 */
    public int[] stageNum = new int[] {56, 40, 82, 74, 60, 92};
    ...
    private void initPaint() {
        ...
        Vector<Integer>getRate= fe.getRate();

        stageNum = new int[] {getRate.elementAt(5),getRate.elementAt(4),getRate.elementAt(3),getRate.elementAt(2),getRate.elementAt(1),getRate.elementAt(0)};
    ...
    private void drawYText(Canvas canvas) {
        ...
        Rect rect = new Rect();
        String longText = unitHeightNum[unitHeightNum.length-1]+"%";  //以最长文字对齐
        textPaint.getTextBounds(longText, 0, longText.length(), rect);

        for (int num : unitHeightNum) {

            canvas.drawText(num + "%", 0, top, textPaint);  //画文字
